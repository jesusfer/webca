...
from webca.crypto.constants import *
...
def create_cert_request(pkey, extensions=[], digest='sha256', **name):
    """
...
def create_certificate(request, issuerCertKey,
                       serial, validityPeriod,
                       digest="sha256"):
    """
    Generate a certificate given a certificate request.
    Arguments: request    - Certificate request to use
...
def create_ca_certificate(name,
    ...
    ca_req = create_cert_request(ca_key, ca_extensions, **name)
    if not signing_cert:
